...
func (p *Mylc) AddBlock(block *Block, slotNumber int64) {
	hash := block.GetHash()
	p.Mutex.Lock()
	...
	if p.State == MylcStateNone {
		p.CurrentSlot = slotNumber
		p.State = MylcStatePrepare
...
func (p *Mylc) ClearState() {
	p.State = MylcStateNone
	p.Mutex.Lock()
...
func (p *Mylc) handlePrepareMessage(msg *Message) {
...
func (p *Mylc) handleCommitMessage(msg *Message) {
	stageMsg := msg.Body.(StageMessage)
	cacheKey := fmt.Sprintf("%s:%d:%s", stageMsg.Hash, stageMsg.Height, stageMsg.Signer)
	...
	if commitInfo != nil {
		if _, ok := commitInfo.Votes[stageMsg.Signer]; !ok {
			...
			//fmt.Println("Mylc", p.Node.ID, "commit votes", commitInfo.VotesNumber, commitInfo.Votes)
...
func (p *Mylc) ProcessStageMessage(msg *Message) {
	switch msg.Type {
